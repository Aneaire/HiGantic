  ...
  return tool(
    ...
    `Update the agent's configuration. Call this whenever the user decides on a name, description, system prompt, model, or tools. You can update one or multiple fields at once. Always call this to save progress as the conversation evolves.`,
    {
      ...
      model: z
        ...
        .optional()
        .describe(
          'The AI model to use. Claude: "claude-sonnet-4-6" (balanced), "claude-opus-4-6" (most capable), "claude-haiku-4-5-20251001" (fastest). Gemini: "gemini-3.1-pro" (most capable), "gemini-3-flash" (fast, agentic), "gemini-3.1-flash-lite" (cheapest), "gemini-2.5-flash" (balanced)'
        ),
      enabledToolSets: z
        .array(z.string())
        .optional()
