  ...
  return tool(
    ...
    `Update the agent's configuration. Call this whenever the user decides on a name, description, system prompt, model, or tools. You can update one or multiple fields at once. Always call this to save progress as the conversation evolves.`,
    {
      ...
      model: z
        ...
        .optional()
        .describe(
          'The AI model to use. Gemini: "gemini-3.1-pro-preview" (most capable), "gemini-3-flash-preview" (fast, agentic), "gemini-2.5-flash" (balanced). OpenAI: "gpt-4o" (flagship), "gpt-4o-mini" (cheap/fast), "o4-mini" (reasoning).'
        ),
      enabledToolSets: z
        .array(z.string())
        .optional()
...
function createUseTemplateTool(ctx: CreatorContext) {
  ...
  return tool(
    "use_template",
    `Apply a starter template to quickly configure the agent. This sets name, description, system prompt, and tool sets all at once (the model stays as the user's default). The user can then customize any part. Available templates:\n${templateList}`,
    {
      templateId: z
        .enum(
