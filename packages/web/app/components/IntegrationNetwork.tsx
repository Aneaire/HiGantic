...
/*
 * Positions pushed to the periphery — nothing in the center 40-80% x / 25-75% y
 * where the 3D logo sits.
 */
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="#FF4A00">
        <path d="M12 2L4 6v6c0 5.55 3.84 10.74 8 12 4.16-1.26 8-6.45 8-12V6l-8-4zm-1 15l-4-4 1.41-1.41L11 14.17l5.59-5.59L18 10l-7 7z"/>
      </svg>
    ),
  },
  {
...
/*
 * Connections — only between adjacent/nearby nodes around the ring.
 * No cross-center lines.
 */
  ...
  return (
    ...
    >
      ...
      {INTEGRATIONS.map((node) => (
        <div
          key={node.id}
          className={`absolute flex items-center justify-center rounded-full bg-zinc-900/80 backdrop-blur-md border ${node.color} shadow-lg network-bubble ${sizeClasses[node.size]}`}
          style={{
            left: `${node.x}%`,
            top: `${node.y}%`,
